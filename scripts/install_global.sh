...
install -d -m 755 "$share_root/config" "$share_root/docs" "$share_root/scripts" "$share_root/portal" "$share_root/systemd" "$share_root/integrations" "$bin_root"
...
git -C "$repo_root" archive HEAD config docs scripts portal systemd integrations README.md CLAUDE.md requirements.txt | tar -x -C "$share_root"
